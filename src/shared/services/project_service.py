...
class ProjectService:
    ...
    def _is_link_valid(self, link_data: Dict[str, Any]) -> bool:
        ...
        try:
            expiration_time = self.parse_datetime_ru(end_at)
            return expiration_time > datetime.now()
        except Exception as e:
            logger.warning("Не удалось распарсить дату окончания ссылки '%s': %s", end_at, e)
            return False
    ...
    async def edit_project(self, proeject_id: int, new_data: ProjectData, user: dict):
        try:
            ...
            res = await self.project_repository.update_project(proeject_id, data_dict)
    ...
    async def add_member(self, link_data: Dict[str, Any], user: Dict[str, Any]) -> bool:
        ...
        if not project_rel:
            logger.warning("Отсутствует 'project_rel' в данных ссылки")
            return False

        project_id = project_rel.get('id')
        if not project_id:
            logger.warning("Отсутствует ID проекта в project_rel")
            return False

        if project_rel.get('status') != 'open':
            return False

        if not self._is_link_valid(link_data):
            return False
        ...
        try:
            ...
            logger.error("Ошибка при добавлении участника в проект %s: %s", project_id, e)
            return False
